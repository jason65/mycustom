<template>
  <div class="kindo-table">
    ...
    <el-table ref="table" @cell-mouse-enter="_mouseEnter" v-bind="$attrs" v-on="$listeners" v-loading="loading" element-loading-background="rgba(255, 255, 255, 0.5)" :height="internalHeight" :data="internalData" :border="internalBorder" :highlight-current-row="internalHighlightCurrentRow" :stripe="internalStripe" @sort-change="internalSortChange">
      <!-- 行号 -->
      <el-table-column v-if="pageIndex" label=" " type="index" align="center" width="50px" class-name="rowNumber" :fixed="pageIndexFixed">
    ...
    <template v-if="pagination">
      <el-pagination class="pagination" background v-bind="$attrs" v-on="$listeners" :layout="internalLayout" :total="internalTotal" :current-page.sync="internalCurrentPage" :page-sizes="internalPageSizes" :page-size="internalPageSize" @size-change="_internalSizeChange" @current-change="_internalCurrentChange">
      </el-pagination>
    </template>
  </div>
</template>
  ...
  mixins: [
    {
      ...
      methods: {
        _mouseEnter() {
          // row, column, cell
          //   if(typeof row[column.property]==='string'&& row[column.property].trim()===''){
          //      cell.innerHTML=''
          //   }else if(row[column.property]===null||row[column.property]===undefined){
          //    cell.innerHTML=''
          //  }
        },
        trigger(method, ...args) {
